...
from core.env_utils import env_default_int, env_default_str
...
MEMORY_PATH = Path(env_default_str("MEMORY_PATH", "./memory"))
try:
    MEMORY_MAX_TURNS = max(1, env_default_int("MEMORY_MAX_TURNS", 10))
except ValueError:
    msg = "Invalid MEMORY_MAX_TURNS - using 10"
    logger.warning(msg)
    print(f"[env] {msg}", flush=True)
    MEMORY_MAX_TURNS = 10
try:
    REDIS_MEMORY_TTL = env_default_int("REDIS_MEMORY_TTL", 0)
except ValueError:
    msg = "Invalid REDIS_MEMORY_TTL - using 0"
    logger.warning(msg)
    print(f"[env] {msg}", flush=True)
    REDIS_MEMORY_TTL = 0
REDIS_URL = os.getenv("REDIS_URL", "").strip()
REDIS_KEY_PREFIX = env_default_str("REDIS_KEY_PREFIX", "rag:thread:")
...
def format_history_for_prompt(turns: list[dict[str, str]]) -> str:
    ...
    for i, t in enumerate(turns):
        u = redact_prompt_injection_spans((t.get("user") or "").strip())
        ...
        if i == last_idx:
            a_display = redact_prompt_injection_spans(a)
        elif len(a) > _HISTORY_ASSISTANT_SUMMARY_LEN:
            a_display = redact_prompt_injection_spans(
                a[:_HISTORY_ASSISTANT_SUMMARY_LEN].rstrip() + " …"
            )
        else:
            a_display = redact_prompt_injection_spans(a)
