...
from agent.text_ar import language_hint_from_text, normalize_arabic_question
...
try:
    MAX_QUESTION_CHARS = max(256, int(os.getenv("GOVERNANCE_MAX_QUESTION_CHARS", "12000")))
except ValueError:
...
try:
    RATE_LIMIT_PER_MINUTE = max(0, int(os.getenv("GOVERNANCE_RATE_LIMIT_PER_MINUTE", "60")))
except ValueError:
...
def evaluate_question(raw: str) -> GovernanceOutcome:
    ...
    for pat in _INJECTION_PATTERNS_AR:
        if pat.search(normalized_scan) or pat.search(text):
            return GovernanceOutcome(False, ("prompt_injection_suspected",), text)
...
def enforce_rate_limit(request: Any) -> None:
    ...
    try:
        from agent.thread_memory import redis_client_optional
