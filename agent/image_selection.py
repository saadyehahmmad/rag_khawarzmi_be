...
from agent.paths import SCREENS_JSON_PATH, load_screens_json, resolve_flow_images
...
SEMANTIC_FLOW_THRESHOLD: float = float(os.getenv("SEMANTIC_FLOW_THRESHOLD", "0.65"))
...
SEMANTIC_FLOW_MARGIN: float = float(os.getenv("SEMANTIC_FLOW_MARGIN", "0.03"))
...
def _ensure_flow_embeddings(flows: list[dict]) -> None:
    ...
    from agent.llm_helpers import get_embeddings  # deferred to avoid circular import
