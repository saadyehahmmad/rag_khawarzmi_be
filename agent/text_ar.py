...
def normalize_arabic_answer_text(text: str) -> str:
    ...
    if not text:
        return text
    # Observed typo in Arabic answers: "القواعس" should be "القواعد".
    # Also observed in headings / rewrites without the definite article.
    return text.replace("القواعس", "القواعد").replace("قواعس", "قواعد")
